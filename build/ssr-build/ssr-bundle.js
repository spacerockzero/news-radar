...
var lodash_sortby = __webpack_require__("9It8");
			...
			Object(preact_min["h"])(
				...
				Object(preact_min["h"])(
					...
					Object(preact_min["h"])(
						...
						Object(preact_min["h"])(
							'a',
							{ href: state.link, name: state.id, onClick: this.props.markAsRead },
							state.title
						)
					),
					Object(preact_min["h"])(
...
// prod
var NEW_ARTICLES_URL = 'https://us-central1-news-radar.cloudfunctions.net/newArticles';
// dev
// const NEW_ARTICLES_URL = 'http://localhost:5000/news-radar/us-central1/newArticles';

function getLocalArticles() {
	var localArticles = void 0;
	try {
...
var articleList__ref = Object(preact_min["h"])(
	preact_material_components_Card,
	null,
	Object(preact_min["h"])(
		preact_material_components_Card.Primary,
...
var articleList_ArticleList = function (_Component) {
	...
	ArticleList.prototype.getPrefs = function getPrefs() {
		var prefs = { topics: ['technology'] };
		try {
	...
	ArticleList.prototype.markAsRead = function markAsRead(e) {
		console.log('e', e);
		// e.preventDefault();
		e.stopPropagation();
		...
		if (id) {
			...
			var read = prefs.read || [];
			read.push(id);
			var uniqRead = lodash_es_uniq(read);
			prefs.read = uniqRead;
			var articles = this.state.articles;
			var filtered = articles.filter(function (art) {
				return !uniqRead.includes(art.id);
			});
	...
	ArticleList.prototype.componentDidMount = function componentDidMount() {
		...
		var localNewArticles = getLocalArticles();
		var read = this.state.preferences.read || [];
		var filtered = localNewArticles.filter(function (art) {
			return !read.includes(art.id);
		});
		if (filtered.length > 0) {
			this.setState({ articles: filtered });
		}
		// queue feed update through firebase function
		var updateFeed = window.fetch(NEW_ARTICLES_URL, {
			method: 'GET',
			mode: 'cors',
			headers: new Headers({
		...
		updateFeed.then(function (data) {
			return data.json();
		}).then(function (newArticles) {
			console.log('newArticles:', newArticles);
			// be sure that articles and uniques is never zero
			var unique = _this2.state.articles.length === 0 && newArticles.length > 0 ? newArticles : lodash_es_differenceBy(_this2.state.articles, newArticles, 'id');
			console.log('unique', unique);
			var filteredUniq = unique.filter(function (art) {
				return read.includes(art.id) === false;
			});
			if (filteredUniq.length > 0) {
				_this2.setState({ newArticles: filteredUniq });
			}
		}).catch(function (err) {
	...
	ArticleList.prototype.render = function render(props, state) {
		...
		var mergeNewArticles = function mergeNewArticles(e) {
			...
			var merged = state.newArticles.concat(state.articles);
			// state.articles.unshift(...state.newArticles);
			_this3.setState({ newArticles: [] });
			_this3.setState({ articles: merged });
		...
		var emptyMessage = articleList__ref;
		return Object(preact_min["h"])(
			'articlelist',
			...
			lodash_map_default()(state.articles, function (article, key, markAsRead) {
				return Object(preact_min["h"])(article_Article, articleList__extends({ key: key }, article, { markAsRead: _this3.markAsRead }));
			})
		);
	};
